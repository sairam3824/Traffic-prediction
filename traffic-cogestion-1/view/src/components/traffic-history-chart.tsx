...
export default function TrafficHistoryChart() {
  ...
  const fetchTrafficHistory = async () => {
    // Demo mode: Generate mock historical and prediction data
    setLoading(true)
    ...
    // Generate historical data (last 10 hours)
    for (let i = 9; i >= 0; i--) {
      const hourTime = new Date(now.getTime() - i * 60 * 60 * 1000)
      const hourKey = hourTime.toISOString().slice(0, 13) + ":00:00.000Z"

      const hour = hourTime.getHours()
      let avgSpeed, avgVolume, congestion
    ...
    // Generate prediction data (next 2 hours)
    for (let i = 1; i <= 2; i++) {
      const futureTime = new Date(now.getTime() + i * 60 * 60 * 1000)
      const hour = futureTime.getHours()

      // Predict based on previous trend + time of day logic
