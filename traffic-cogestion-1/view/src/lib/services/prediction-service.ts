...
import { Prediction } from "@/lib/db/traffic"
...
export class PredictionService {
  ...
  async getLatestPredictions(segmentId: string, modelType: string, limit = 24): Promise<Prediction[]> {
    const supabase = await createClient()
  ...
  async comparePredictions(segmentId: string): Promise<Record<string, Prediction[]>> {
