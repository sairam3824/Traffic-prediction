...
import { TrafficSegment } from "@/lib/db/traffic"
...
export class AlertGenerationService {
  ...
  private async checkSegmentPredictions(segment: TrafficSegment): Promise<void> {
    try {
      const supabase = await createClient()
      ...
      if (this.alertService.shouldCreateCongestionAlert(prediction.predicted_speed_kmh, prediction.predicted_volume)) {
        ...
        if (!existingAlerts || existingAlerts.length === 0) {
          ...
          await this.notificationService.sendNotification({
            ...
            severity: severity as NotificationPayload["severity"],
