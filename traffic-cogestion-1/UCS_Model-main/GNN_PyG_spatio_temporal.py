...
class SpatioTemporalDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for t in range(T - SEQ_LEN - HORIZON + 1):
            x = torch.tensor(self.tensor[t:t+SEQ_LEN], dtype=torch.float)
            y = torch.tensor(self.tensor[t+SEQ_LEN+HORIZON-1], dtype=torch.float)
            x_nodes = x.permute(1,0,2).reshape(Z, -1)
...
class STGCN(nn.Module):
    ...
    def forward(self, data):
        ...
        for b in range(batch):
            ...
            xb_g = self.gcn1(xb, edge)
            out_nodes.append(xb_g.unsqueeze(0))
        out = torch.cat(out_nodes, dim=0)
